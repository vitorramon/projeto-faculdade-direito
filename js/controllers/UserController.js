class UserController {
  ...
  addLine(user) {
    ...
    document
      ...
      [document.querySelectorAll(".edit-btn").length - 1].addEventListener(
        "click",
        () => {
    ...
    document
      ...
      [document.querySelectorAll(".delete-btn").length - 1].addEventListener(
        "click",
        () => {
  ...
  register() {
    ...
    if (JSON.stringify(this.users) == JSON.stringify({})) {
      ...
      let lastUserObj = Object.values(this.users)[
        Object.values(this.users).length - 1
      ];
      ...
      if (lastUser.getId() == "mestre") {
        if (Object.values(this.users).length == 1) {
          ...
          let lastUserObj = Object.values(this.users)[
            Object.values(this.users).length - 2
          ];
          let lastUser = new User(
            lastUserObj._id,
            lastUserObj._name,
  ...
  edit() {
    ...
    let selectedUser = [
      ...document.querySelectorAll(".users tr:not(:first-child)"),
    ].filter((v) => {
    ...
    if (files.length == 0) {
      ...
      this.attLoginData();
      if (
        (this.login.getId() == user.getId() &&
          this.login.getEmail() != user.getEmail()) ||
        user.getAdmin() == false
      ) {
